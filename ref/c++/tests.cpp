...
static std::string to_lc(const std::string& str) {
    std::string ret;
    ret.resize(str.size());
    for (unsigned int p = 0; p < str.size(); ++p) {
        char c = str[p];
        ret[p] = c >= 'A' && c <= 'Z' ? (c - 'A') + 'a' : c;
    }
    return ret;
}
...
int main(void) {
    ...
    for (i = 0; i < sizeof(valid_checksum) / sizeof(valid_checksum[0]); ++i) {
        int ok =1;
        std::pair<std::string, bech32::data> dec = bech32::decode(valid_checksum[i]);
        if (dec.first.empty()) {
            printf("bech32_decode fails: '%s'\n", valid_checksum[i].c_str());
            ok = 0;
        }
        if (ok) {
            std::string enc = bech32::encode(dec.first, dec.second);
            printf("a=%s b=%s\n", valid_checksum[i].c_str(), enc.c_str());
            if (enc != valid_checksum[i] && to_lc(enc) != valid_checksum[i]) ok = 0;
        }
        fail += !ok;
    }
